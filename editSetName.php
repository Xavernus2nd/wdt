...
if(isset($_POST['NewSetName'])){
    ...
    $checkTeacher = mysqli_fetch_assoc(mysqli_query($conn, "SELECT TeacherUsername FROM QuestionSet WHERE SetID = '$targetSetID'"));
    ...
    if ($TeacherUsername!=$targetSetTeacherName){
        ...
        if (mysqli_query($conn, "UPDATE QuestionSet SET SetName = '$NewSetName' WHERE SetID = '$targetSetID' AND TeacherUsername = '$TeacherUsername'")) {
